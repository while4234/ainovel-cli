...
- Latest source change: branch `main`, commit `389b0ab`
  ...
  Web assets. The larger two-stage volume-review state machine remains a
...
- Working tree: expected clean after the proposal-review UI checkpoint,
  except for
  ignored local `.codex/`, `.playwright-mcp/`, and output/runtime artifacts.
- Runtime: local Web static assets were regenerated by
  `npm --prefix internal\entry\web\ui run build`; local Web was restarted on
  `http://127.0.0.1:9898` for browser testing.
- Validation: `go test ./...` passed; `npm --prefix internal\entry\web\ui test
  ...
  `git diff --check` passed with Windows LF/CRLF warnings only; Playwright
  verified the saved-proposal review workspace and single/range/volume controls
  on a safe local test project.
