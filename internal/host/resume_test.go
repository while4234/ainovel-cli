...
func TestPrepareResumeOutlineRepairQueuesCompletedBatchRewrites(t *testing.T) {
	...
	if result.Batch.Volume != 1 || result.Batch.Arc != 2 {
		t.Fatalf("expected V1 A2, got V%d A%d", result.Batch.Volume, result.Batch.Arc)
	}
	if len(result.QueuedChapters) != 1 || result.QueuedChapters[0] != 3 {
		t.Fatalf("queued chapters = %v, want [3]", result.QueuedChapters)
	}
	...
	if len(progress.PendingRewrites) != 2 || progress.PendingRewrites[0] != 3 || progress.PendingRewrites[1] != 2 {
		t.Fatalf("pending rewrites = %v, want [3 2]", progress.PendingRewrites)
	}
	if progress.RewriteReason == "old polish" || progress.RewriteReason == "" {
		t.Fatalf("rewrite reason was not updated: %q", progress.RewriteReason)
	}
}
...
func TestDescribeResumeShowsOutlineRepairBeforeRewriteQueue(t *testing.T) {
	...
	if label := describeResume(st, progress); label != "恢复：重复大纲待修复（V1 A2）" {
