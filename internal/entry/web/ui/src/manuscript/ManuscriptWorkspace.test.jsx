...
describe('ManuscriptWorkspace', () => {
  it('loads completed prose while writing and exposes ARIA tree/tabs/current-candidate copy', async () => {
    ...
    await act(async () => root.render(<ManuscriptWorkspace projectId="p" />));
    await act(async () => [...container.querySelectorAll('button')].find((b) => b.textContent === '专业长篇稿件工作区').click()); await settle();
    expect(container.querySelector('[role="tree"]')).not.toBeNull();
    expect(container.querySelector('[role="tablist"]')).not.toBeNull();
    expect(container.textContent).toContain('当前正式稿'); expect(container.textContent).toContain('候选稿（尚未替换当前稿）');
    expect(container.textContent).toContain('c1-current'); expect(container.textContent).toContain('c1-candidate');
  });
  ...
  it.each([
    ...
    try {
      await act(async () => root.render(<ManuscriptWorkspace projectId="p" />));
      await act(async () => container.querySelector('.manuscript-workspace-toggle').click()); await settle();
  ...
  it('aborts and isolates all old project state when projectId changes', async () => {
    ...
    await act(async () => root.render(<ManuscriptWorkspace projectId="A" />));
    await act(async () => container.querySelector('.manuscript-workspace-toggle').click()); await settle();
    await act(async () => root.render(<ManuscriptWorkspace projectId="B" />)); await settle();
  ...
  it('keeps mobile drawer focus inside, closes with Escape, and restores the opener', async () => {
	...
	it('renders top-level human chips and never forwards an internal context object', async () => {
		...
		await act(async () => root.render(<ManuscriptWorkspace projectId="p" onDiscussionReady={onDiscussionReady} />));
		await act(async () => container.querySelector('.manuscript-workspace-toggle').click()); await settle();
	...
	it('cursor-loads one signed history version and keeps its DOM window bounded', async () => {
		...
		await act(async () => [...container.querySelectorAll('button')].find((button) => button.textContent === '继续加载正文').click()); await settle();
		expect(container.textContent).toContain('共已加载 240 段');
