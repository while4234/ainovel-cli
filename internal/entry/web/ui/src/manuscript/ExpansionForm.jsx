export function ExpansionForm({ value, onChange, onSubmit, busy, mode }) {
  return <form className="expansion-form" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
    ...
    <label>一句话描述<textarea required maxLength="500" rows="2" value={value.sentence} onChange={(event) => onChange({ ...value, sentence: event.target.value })} placeholder="例如：让主角发现盟友隐瞒的证据，并被迫公开站队" /></label>
    ...
    <button type="submit" disabled={busy || !value.sentence.trim()}>{busy ? '规划中…' : '让 AI 规划'}</button>
  </form>;
}
