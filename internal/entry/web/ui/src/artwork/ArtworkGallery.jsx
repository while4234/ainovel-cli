import { CheckCircle2, Download, Expand, ImageOff, RotateCw, Trash2, XCircle } from 'lucide-react';
import { useState } from 'react';
import { ArtworkConfirmDialog } from './ArtworkDialogs.jsx';

export function ArtworkGallery({ assets, cursor, operation, onApply, onUnapply, onDelete, onDownload, onLoadMore, onReuse }) {
  ...
  return <section className="artwork-gallery" aria-labelledby="artwork-gallery-title">
    ...
    <div className="artwork-gallery-grid">
      {assets.map((asset) => <article className={asset.applied ? 'applied' : ''} key={asset.id}>
        <button aria-label={`预览图片 ${asset.id}`} className="artwork-image-preview" onClick={() => setPreview(asset)} type="button">
          <img alt={asset.work_type === 'character_portrait' ? '角色肖像' : asset.work_type === 'cover' ? '书籍封面' : '小说插图'} loading="lazy" src={asset.applied_content_url || asset.content_url} />
        ...
        <div className="artwork-asset-actions">
          ...
          {asset.applied
            ? <button aria-label="取消应用图片" disabled={Boolean(operation)} onClick={() => onUnapply(asset)} title="取消应用" type="button"><XCircle size={15} /></button>
            : <button aria-label="应用图片" disabled={Boolean(operation)} onClick={() => onApply(asset)} title="应用" type="button"><CheckCircle2 size={15} /></button>}
    ...
    {preview ? <div className="artwork-preview-backdrop" onMouseDown={() => setPreview(null)} role="presentation"><div aria-label="图片预览" aria-modal="true" className="artwork-preview-dialog" onMouseDown={(event) => event.stopPropagation()} role="dialog"><img alt="绘境大图预览" src={preview.applied_content_url || preview.content_url} /><button className="artwork-button subtle" onClick={() => setPreview(null)} type="button">关闭预览</button></div></div> : null}
