...
export function normalizeFoundationResponse(response) {
  ...
  return {
    ...
    sourceAnalysis: state.source_analysis || null,
    targetFoundation: normalizeFoundation(state.target_foundation),
    targetAvailable: Boolean(state.target_available),
    targetCompleteness: state.target_completeness || null,
    recoveryAvailable: Boolean(state.recovery_available),
    nextActions: array(state.next_actions).map(String),
