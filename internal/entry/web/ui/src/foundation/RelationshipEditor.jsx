...
export function RelationshipEditor({ projectId, auditSignature, coreCast, value, characters, reviewed, disabled, errors = {}, onChange, onReviewedChange }) {
	...
	if (disabled) return <section aria-labelledby="foundation-relationship-heading"><div className="foundation-section-head"><h2 id="foundation-relationship-heading">计划关系（只读）</h2></div>{!value.length ? <div className="empty-state">已审查：当前没有计划关系。</div> : <div className="foundation-editor-list">{value.map((relationship, index) => <article className="foundation-editor-card readonly-card" key={relationship.id || index}><h3>{relationship.label || `关系 ${index + 1}`}</h3><p><strong>{characters.find((character) => character.id === relationship.source_character_id)?.name || relationship.source_character_id || '—'}</strong> → <strong>{characters.find((character) => character.id === relationship.target_character_id)?.name || relationship.target_character_id || '—'}</strong></p><p>{relationship.type} · {relationship.direction} · {relationship.status}</p><p>{relationship.description || '—'}</p></article>)}</div>}</section>;
