...
func (s *Server) handleProjectAdaptAnalyze(w http.ResponseWriter, r *http.Request, id string) {
	...
	legacyIncrementalForce := analyzeRequest.Force && strings.TrimSpace(analyzeRequest.Mode) == ""
	if mode == "complete_missing" && status.AnalysisStatus == "done" && !legacyIncrementalForce {
