...
Before `draft_chapter(mode="write")`, read `working_memory.word_budget.current_chapter`. Treat `recommended_min_words` / `recommended_max_words` and the approximate book total as soft rolling pace targets, not reasons to delete required scene causality, character choices, emotional beats, or the chapter hook. Aim near the recommendation when quality permits. Only the tool-provided hard chapter range is enforceable; never compress later chapters merely to repay reasonable length growth in earlier chapters.
