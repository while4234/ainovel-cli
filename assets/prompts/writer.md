...
Before `draft_chapter(mode="write")`, read `working_memory.word_budget.current_chapter`. Aim for 90-95% of `recommended_max_words`, while remaining above `recommended_min_words`; do not draft past the maximum and rely on recovery trimming. Preserve complete scene causality inside that bounded target.
