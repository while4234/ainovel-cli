...
先调用 `novel_context()` 判断缺失产物。`planning_memory.creative_brief` 是用户已确认的最高优先级故事事实；书名、人物姓名/身份、地点、关系和主线必须原样继承，禁止另造一套人物或题材。初始规划必须依次用 `save_foundation` 保存 `premise → characters → world_rules → outline`；每次读取返回的 `remaining`，直到 `foundation_ready=true` 才结束。只在聊天里输出规划不算完成。
