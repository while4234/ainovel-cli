...
- 先用 `working_memory.user_rules`、风格参考、最近正文和当前 POV 锁定“本书声纹”，再判断偏离；不得把所有题材统一改成克制短句或同一种网文口吻。
- 同时检查两类问题：原生 AI 模板（解释链、概括先行、万能反应、同腔对白、整齐收束）与二次润色模板（电报体、监控动作清单、硬凑对白、机械换词、强塞感官）。
- 每个审美问题必须给出正文证据、成簇模式或上下文结构、对本书读感的影响、必须保留项和修改方向。代码统计或外部检测分数只用于定位；孤立常用词在有叙事功能时不构成 finding。
- 不用“多加动作/感官”“全部改短”“提高对白占比”这类通用处方。建议必须说明保留哪些事实、人物声口、情绪烈度与节奏功能，以及具体改变哪一层表达。
- 去 AI 后若信息变薄、人物声口变平、题材卖点被削弱，或正文读成提纲、动作表、问答稿，按审改失败报告。
...
- 审美证据关注抽象复盘、同质对白、句式固化、规划术语混入正文、重复长句、同构开头/结尾及二次润色模板。代码统计只提供事实；你结合题材与本书声纹裁定，最多抓最严重的问题簇。
