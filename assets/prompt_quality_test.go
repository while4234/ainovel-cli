	...
	policies := []struct {
	...
	}{
		{
			...
			policy: promptreview.Policy{Role: "writer", MaxTokens: 2500, ForbiddenPhrases: []string{"Chapter 模式", "Arc 模式", "Free 模式"}, Capabilities: []promptreview.Capability{
				...
				{ID: "state", Description: "causal and relationship state continuity", AnyOf: []string{"关系变化由事件与选择推动"}},
				{ID: "anti_ai", Description: "human-like prose correction", AnyOf: []string{"先收紧泛化解释与空泛意义"}},
				{ID: "scene_emotion", Description: "emotion uses multiple scene channels without forcing physiology", AnyOf: []string{"动作、选择、说话方式、感知或贴身念头"}},
				{ID: "dialogue_voice", Description: "dialogue reflects identity and pressure", AnyOf: []string{"身份、利益、知识边界与当下压力"}},
				{ID: "style_fidelity", Description: "drafting preserves the current book voice", AnyOf: []string{"本书声纹"}},
				{ID: "scene_feedback", Description: "scene progression uses visible causal feedback", AnyOf: []string{"场景以可见反馈接力"}},
				{ID: "three_pass_repair", Description: "de-AI reread follows the fused three-pass method", AnyOf: []string{"回读采用三遍法"}},
				{ID: "repair_preservation", Description: "local repair protects story and voice functions", AnyOf: []string{"事实、声口、情绪、关系、伏笔与节奏功能"}},
		...
		},
		{
			...
			policy: promptreview.Policy{Role: "editor", MaxTokens: 2500, ForbiddenPhrases: []string{"Chapter 模式", "Arc 模式", "Free 模式"}, Capabilities: []promptreview.Capability{
				...
				{ID: "style_fidelity", Description: "review is calibrated to the current book voice", AnyOf: []string{"本书声纹"}},
				{ID: "de_ai_evidence", Description: "detector scores only locate evidence", AnyOf: []string{"代码统计与外部检测结果负责定位"}},
				{ID: "actionable_protection", Description: "recommendations name protected narrative functions", AnyOf: []string{"必须保留的叙事功能和具体修订目标"}},
				{ID: "acceptance_voice", Description: "accepted edits still read as the same novel", AnyOf: []string{"成稿连读仍像同一本小说"}},
			}},
		},
		{
