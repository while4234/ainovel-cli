	...
	policies := []struct {
	...
	}{
		{
			...
			policy: promptreview.Policy{Role: "writer", MaxTokens: 2500, ForbiddenPhrases: []string{"Chapter 模式", "Arc 模式", "Free 模式"}, Capabilities: []promptreview.Capability{
				...
				{ID: "scene_emotion", Description: "emotion uses multiple scene channels without forcing physiology", AnyOf: []string{"动作、对白、感官、念头和选择共同承载情绪"}},
				...
				{ID: "anti_overcorrection", Description: "de-AI edits must not become a second template", AnyOf: []string{"电报体、监控动作清单、硬凑对白"}},
				{ID: "no_style_quotas", Description: "scene function controls cadence instead of quotas", AnyOf: []string{"没有句长、段长、对白占比、感官数量配额"}},
				{ID: "repair_preservation", Description: "local repair protects story and voice functions", AnyOf: []string{"必须保留的事实、人物声口、情绪强度"}},
		...
		},
		{
			...
			policy: promptreview.Policy{Role: "editor", MaxTokens: 2500, ForbiddenPhrases: []string{"Chapter 模式", "Arc 模式", "Free 模式"}, Capabilities: []promptreview.Capability{
				...
				{ID: "two_sided_de_ai", Description: "review catches both native and overcorrected AI templates", AnyOf: []string{"原生 AI 模板", "二次润色模板"}},
				{ID: "de_ai_evidence", Description: "detector scores only locate evidence", AnyOf: []string{"代码统计或外部检测分数只用于定位"}},
				{ID: "actionable_protection", Description: "recommendations name protected narrative functions", AnyOf: []string{"必须保留项和修改方向"}},
			}},
		},
		{
